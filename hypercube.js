function hypercube() {
  ...
  window.HYPERCUBE_DEFAULT_OPTIONS = window.HYPERCUBE_DEFAULT_OPTIONS || { 'edges': true, 'indices': false, 'perspective': true, 'rotate_zw': true, 'rotate_yw': true, 'rotate_xz': true};
    ...
    hypercube.draw = function()
    {
      ...
      for (var i in this.vertices)
      {
        ...
        else
        {
          adjusted[i] =
          {
            x: canvas.width / 2 + this.bound * (this.vertices[i].x / this.maxNorm) + 0.5,
            y: canvas.height / 2 - this.bound * (this.vertices[i].y / this.maxNorm) + 0.5,
            z: Math.abs( 0.35 + 0.65 * this.vertices[i].z / this.maxNorm),
            w: 151 + Math.floor(40 * this.vertices[i].w)
          };
        }
      }
